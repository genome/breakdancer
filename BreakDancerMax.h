...
void buildConnection(map<string,vector<int> > &read, map<int,vector<int> > &reg_name, map<int,vector<vector<string> > > &regs, int min_read_pair, string dump_BED, int max_readlen, map<string,string> SVtype, string prefix_fastq, map<string,ofstream> ReadsOut, map<uint32_t,string> SVtype);
